...
public class TrainScenes {
    public static void signaling(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showControls(
                util.vector().blockSurface(signal1, Direction.EAST), Pointing.RIGHT,40).rightClick()
...
//        scene.rotateCameraY(85);
    ...
    private static void initSwitch(CreateSceneBuilder scene, BlockPos switchPos, PonderData data) {
    ...
    private static void clearSwitch(CreateSceneBuilder scene, BlockPos switchPos) {
        scene.addInstruction(new PonderInstruction() {
            @Override
            public boolean isComplete() {
                return true;
    ...
    private static void setSwitchState(CreateSceneBuilder scene, BlockPos switchPos, SwitchState state) {
        scene.addInstruction(new PonderInstruction() {
            @Override
            public boolean isComplete() {
                return true;
    ...
    public static void coupling(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay()
            .showControls(couplerPad1TopMarker, Pointing.DOWN, 40).rightClick()
        ...
        scene.overlay()
            .showControls(couplerTop, Pointing.DOWN,60).scroll()
                .withItem(AllItems.WRENCH.asStack());
        ...
        scene.overlay().showControls(couplerTop, Pointing.DOWN, 60).rightClick()
                .withItem(AllItems.WRENCH.asStack());
        scene.idle(5);
    ...
    /*public static void couplingAlignment(CreateSceneBuilder scene, SceneBuildingUtil util) {
    ...
    public static void movePlate(CreateSceneBuilder scene, SceneBuildingUtil util, BlockPos couplerPos, BlockPos plate, int idleTicks) {
    ...
    public static void coupleTrain(CreateSceneBuilder scene, BlockPos pos, double distance, Direction direction) {
    ...
    public static void decoupleTrain(CreateSceneBuilder scene, BlockPos pos) {
        scene.addInstruction(PonderInstruction.simple(ponderScene -> {
            PonderLevel world = ponderScene.getWorld();
            world.getBlockEntity(pos, AllBlockEntityTypes.BOGEY.get()).ifPresent(sbte -> {
                if (sbte instanceof IStandardBogeyTEVirtualCoupling virtualCoupling) {
